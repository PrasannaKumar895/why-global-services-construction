...
export const Project = () => {
    ...
    return (
        <div className='bg-gray-800 z-50 mt-5 lg:p-1.5 md:1.5 p-4 lg:h-[570px] h-full'>
            <div className="max-w-6xl mx-auto py-1 px-4 md:px-8 lg:px-6">
                ...
                <div className="flex m-[-15px] flex-col lg:flex-row md:flex-row lg:items-start lg:justify-around md:items-start md:justify-around justify-center items-center lg:gap-2 md:gap-1 gap-6">
                    ...
                    <div className=' md:w-1/2 lg:w-[40%] w-[77%] flex items-center justify-center'>
                        <div 
                            className=' w-full h-[250px]  md:h-[390px] lg:h-[370px] bg-cover drop-shadow-[0_4px_4px_rgba(255,255,255,0.9)] filter  bg-center rounded-xl'
                            style={{ 
                    ...
                    <div className='mb-4 flex  lg:px-1 md:px-5 px-10 flex-col lg:gap-6 gap-3 md:w-1/2 lg:[60%] '>
                        {projects.map((item, index) => (
                            <div
                                key={index}
                                className='py-1 px-6 rounded-xl bg-gray-200 hover:bg-white shadow-lg border-l-8 border-orange-600 '
                                onMouseEnter={() => setHoverIndex(index)}
                                onMouseLeave={() => setHoverIndex(null)}
                            >
                                <h3 className='text-md text-black font-bold mb-1'>{item.title}</h3>
                                <p className='text-sm text-justify text-black mb-2'>{item.description}</p>
                                <button className='cursor-pointer py-0.5 px-4 rounded-sm bg-orange-600 text-white hover:bg-gray-100 border hover:text-orange-600 hover:border hover:border-orange-600  transition-colors duration-1000 text-sm '>
                                    More
                                </button>
                            </div>
                        ))}
  ...
  <button className='text-white  rounded-md px-4 py-2 border-2 border-orange-600  hover:bg-white hover:font-bold hover:text-orange-600 transition-colors duration-300'>
    View More
  </button>
</div>
